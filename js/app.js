    ...
    function CreateConnection(connection){
        ...
        var width = Math.abs(end_pos.x - start_pos.x);
         ...
         ctx.bezierCurveTo(width/2, 0, width/2, height, width, height);
         ctx.stroke();

    }

    function ConnectionClick(event) {
        if(event.target.getAttribute("class").indexOf("pin") > -1)
        {


           if(!event.currentTarget.hasAttribute("id") && Connecting) 
            {
            Connecting = false; 
            ConnectionSettings.End = [event.clientX, event.clientY];
            ...
            console.log(ConnectionSettings.StartPin.id + ConnectionSettings.EndPin.id);
            ...
            else if (!event.currentTarget.hasAttribute("id"))
            {
            Connecting = true;
            ConnectionSettings.Start = [event.clientX, event.clientY];
